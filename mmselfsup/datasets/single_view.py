...
class SingleViewDataset(BaseDataset):
    ...
    def __init__(self, data_source, pipeline, prefetch=False, **kwargs):
            ...
            if not per_class:
                ...
                for label in range(n_classes):
                    ...
                    for k in topk:
                        ...
                        acc = correct_k * 100.0 / num
                        class_weight = len(target_class_labels) / len(target)
                        recall = correct_k * 100.0 / len(target_class_labels)
                        recalls.append(recall)
                        # print(f"class {label} weight: {class_weight}")
                        # print(f"class {label} recall: {recall}")
                        weighted_recall += class_weight * recall
                        mean += acc
                        accuracies.append(acc)
                        # if logger is not None and logger != 'silent':
                        #     print_log(
                ...
                eval_res[f'val_balanced_recall'] = weighted_recall
                # eval_res[f'top_1_acc'] = accuracies[0]
                for idx, recall in enumerate(recalls):
                    eval_res[f'class_{idx}_recall'] = recall
                print(eval_res)
                # tune.report(mean_acc=mean/n_classes)
            # tune.report(acc=accuracies[0])
