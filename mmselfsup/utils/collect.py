...
def nondist_forward_collect_tta(func, data_loader, length):
    results = []
...
def nondist_forward_collect_test_loss(func, data_loader, length):
    ...
    results_all = {}
    for k in results[0].keys():
        results_all[k] = [batch[k].cpu().item() for batch in results]
    return results_all
