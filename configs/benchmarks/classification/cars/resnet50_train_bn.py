...
model = dict(head=dict(num_classes=n_classes, type='ClsHeadWeightedNonLinear', mlp=False, weight=class_imbalances, dropout_rate=0.0, with_avg_pool=True, in_channels=2048, bn_only=False), backbone=dict(frozen_stages=4, unfreeze_bn=False))

optimizer = dict(type='SGD', lr=0.01)

# img_norm_cfg = dict(mean=[0.0, 0.0, 0.0], std=[1.0, 1.0, 1.0])
runner = dict(type='EpochBasedRunner', max_epochs=1)
evaluation = dict(interval=1, start=10, save_best="auto", topk= (1,), per_class=True, n_classes=n_classes)
